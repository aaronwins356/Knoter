...
4. (Optional) Override the base URL with `KALSHI_BASE_URL` if Kalshi updates production endpoints.
5. Set `KNOTER_LIVE_TRADING_ENABLED=true` in your `.env` and restart the backend.
6. In the dashboard, switch mode to LIVE and type the confirmation phrase **ENABLE LIVE TRADING**.
...
Knoter now normalizes **dollar-denominated** fields first (`yes_bid_dollars`, `yes_ask_dollars`, `no_bid_dollars`,
`no_ask_dollars`) and logs a warning if legacy cent fields are used. All internal prices are stored as floats in the
`[0.01, 0.99]` dollar range.

## Environments

- `KALSHI_ENV=demo` → https://demo-api.kalshi.co/trade-api/v2
- `KALSHI_ENV=live` → https://api.kalshi.com/trade-api/v2 (override with `KALSHI_BASE_URL`)

## API Endpoints

- `GET /health` — service + connection status
- `GET /kalshi/status` — Kalshi connectivity, environment, and masked account info
- `GET /kalshi/markets/windowed` — authenticated, windowed market discovery
- `GET /kalshi/markets/{ticker}/quote` — authenticated market quote lookup
